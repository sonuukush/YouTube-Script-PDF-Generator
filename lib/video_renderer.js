 ...
 * and automatic disk space cleanup.
 */
...
async function renderKineticVideo(scenes, imagePaths, aspectRatio, outputVideoPath, jobTempDir, onProgress) {
    ...
    return new Promise((resolve, reject) => {
        ffmpeg()
            ...
            .on('end', () => {
                ...
                // Automatic Temp Disk Cleanup
                try {
                    if (fs.existsSync(jobTempDir)) {
                        fs.rmSync(jobTempDir, { recursive: true, force: true });
                        console.log(`[VideoRenderer Cleanup] Purged temp directory: ${jobTempDir}`);
                    }
                } catch (cleanupErr) {
                    console.warn(`[VideoRenderer Cleanup Warning] Failed to clean ${jobTempDir}: ${cleanupErr.message}`);
                }
